...
int ip_counts, tcp_counts, udp_counts, dns_counts, icmp_counts;
...
void alarm_handler(int sig)
{
    ...
    ofstream oFile;
     //打开zhidao要输出的文件
    oFile.open("sniffer_hwang85.csv", ios::out | ios::trunc);
    oFile << "ip," << ip_counts << endl;
    oFile << "tcp," << tcp_counts << endl;
    oFile << "udp," << udp_counts << endl;
    oFile << "dns," << dns_counts << endl;
    oFile << "icmp," << icmp_counts << endl;
    oFile.close();

    printf("ip: %d  tcp: %d  udp: %d  dns: %d  icmp: %d\n", ip_counts, tcp_counts, udp_counts, dns_counts, icmp_counts);
    exit(0);
}
...
//This packet is UDP.
      ...
      /* define/compute udp header offset */
        ...
        if(ntohs (udp->sport) == 53 || ntohs (udp->dport) == 53)
            dns_counts++;
